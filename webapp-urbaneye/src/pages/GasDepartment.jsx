...
const statusCfg = {
    ...
    warning: { label: 'Low Pressure', dot: '#f59e0b', badge: { bg: '#fffbeb', color: '#d97706', border: '#fde68a' } },
    overpressure: { label: 'Overpressure', dot: '#ef4444', badge: { bg: '#fef2f2', color: '#dc2626', border: '#fecaca' } },
};
...
const GasDepartment = () => {
    ...
    const steps = ['Connecting to IGL SCADA system…', 'Polling pipeline pressure sensors…', 'Analysing leak probability models…', 'Generating Delhi safety report…'];
            ...
            <SmartCitySidebar>
                <div style={{ minHeight: '100vh', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ textAlign: 'center', maxWidth: 360 }}>
                        <div style={{ width: 56, height: 56, border: '3px solid #fed7aa', borderTopColor: '#f97316', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 24px' }} />
                        <div style={{ fontSize: 18, fontWeight: 700, color: '#1e293b', marginBottom: 4 }}>Gas Safety Command</div>
                        <div style={{ fontSize: 13, color: '#94a3b8', marginBottom: 20 }}>IGL Pipeline Network — Delhi NCR</div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, textAlign: 'left' }}>
                            {steps.map((s, i) => (
                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, opacity: i <= loadingStep ? 1 : 0.25, transition: 'opacity .4s' }}>
                                    {i < loadingStep ? <CheckCircle size={14} color="#16a34a" /> : i === loadingStep ? <div style={{ width: 14, height: 14, border: '2px solid #fed7aa', borderTopColor: '#f97316', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /> : <div style={{ width: 14, height: 14, borderRadius: '50%', border: '1px solid #cbd5e1' }} />}
            ...
            <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
                ...
                {delhiPipelines.some(p => p.status === 'overpressure') && (
                    <div style={{ background: '#ef4444', padding: '10px 24px', color: '#fff', fontSize: 13, display: 'flex', alignItems: 'center', gap: 10 }}>
                        <AlertOctagon size={15} /> <strong>CRITICAL:</strong> Overpressure detected on IGL-E-07 (Kondli). Auto relief valve activated.
                ...
                <div style={{ borderBottom: '1px solid #f1f5f9', background: '#fff', position: 'sticky', top: 0, zIndex: 40 }}>
                    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                            <div style={{ width: 40, height: 40, background: '#f97316', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Flame size={20} color="#fff" /></div>
                            <div>
                                <h1 style={{ fontWeight: 700, fontSize: 18, color: '#0f172a', margin: 0 }}>Gas Department</h1>
                                <p style={{ fontSize: 12, color: '#94a3b8', margin: 0, display: 'flex', alignItems: 'center', gap: 4 }}><MapPin size={10} />IGL Pipeline Network — Delhi NCR</p>
                            </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            <span style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 12px', background: '#fff7ed', color: '#ea580c', borderRadius: 20, fontSize: 12, fontWeight: 600, border: '1px solid #fed7aa' }}>
                                <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#f97316' }} />SCADA Live
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ maxWidth: 1200, margin: '0 auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
                        {[
                            { label: 'Pipeline Zones', value: delhiPipelines.length, sub: 'Delhi monitored', color: '#f97316', icon: Activity },
                            { label: 'Safety Alerts', value: delhiPipelines.filter(p => p.status !== 'normal').length, sub: 'Needs attention', color: '#ef4444', icon: AlertOctagon },
                            { label: 'Active Incidents', value: incidents.filter(i => i.status !== 'Resolved').length, sub: 'Responders deployed', color: '#f59e0b', icon: Phone },
                            { label: 'Network Safety', value: `${Math.round(delhiPipelines.filter(p => p.status === 'normal').length / delhiPipelines.length * 100)}%`, sub: 'Zones clear', color: '#10b981', icon: Shield },
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            <h2 style={{ fontWeight: 700, fontSize: 15, color: '#0f172a', display: 'flex', alignItems: 'center', gap: 8, margin: 0 }}><Activity size={16} color="#f97316" />Pipeline Zone Monitoring — Live Pressure</h2>
                            {delhiPipelines.map((zone, i) => {
                                ...
                                return (
                                    <div key={zone.id} onClick={() => setSelected(selected === zone.id ? null : zone.id)}
                                        style={{ background: '#fff', borderRadius: 12, border: `1px solid ${selected === zone.id ? '#fed7aa' : '#f1f5f9'}`, cursor: 'pointer', transition: 'border-color .15s' }}>
                                        <div style={{ padding: 20 }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                                                <div>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                                                        <span style={{ width: 8, height: 8, borderRadius: '50%', background: cfg.dot }} />
                                                        <span style={{ fontWeight: 700, color: '#0f172a', fontSize: 14 }}>{zone.pipeline}</span>
                                                ...
                                                <div style={{ textAlign: 'right' }}>
                                                    ...
                                                    <div style={{ fontSize: 11, color: '#94a3b8' }}>live pressure</div>
                                                </div>
                                            </div>
                                            ...
                                            {selected === zone.id && (
                                                <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #f1f5f9' }}>
                                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                                                        <div style={{ background: '#f8fafc', borderRadius: 8, padding: 12 }}><div style={{ fontSize: 11, color: '#94a3b8' }}>Zone ID</div><div style={{ fontWeight: 700, fontSize: 13, color: '#0f172a', fontFamily: 'monospace' }}>{zone.id}</div></div>
                                                    ...
                                                    <div style={{ display: 'flex', gap: 8 }}>
                                                        <button style={{ flex: 1, padding: '10px 0', background: '#f97316', border: 'none', color: '#fff', fontSize: 13, fontWeight: 600, borderRadius: 8, cursor: 'pointer' }}>Dispatch Inspector</button>
                                                        <button style={{ flex: 1, padding: '10px 0', background: '#ef4444', border: 'none', color: '#fff', fontSize: 13, fontWeight: 600, borderRadius: 8, cursor: 'pointer' }}>Emergency Isolate</button>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f1f5f9', padding: 20 }}>
                                <h3 style={{ fontWeight: 700, fontSize: 14, color: '#0f172a', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8, margin: '0 0 14px' }}><Flame size={15} color="#f97316" />Active Incidents</h3>
                                {incidents.map(inc => (
                            ...
                            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f1f5f9', padding: 20 }}>
                                <h3 style={{ fontWeight: 700, fontSize: 14, color: '#0f172a', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8, margin: '0 0 14px' }}><Shield size={15} color="#10b981" />Safety Systems</h3>
                                {[
                            ...
                            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #fed7aa', padding: 20 }}>
                                <h3 style={{ fontWeight: 700, fontSize: 14, color: '#ea580c', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8, margin: '0 0 12px' }}><Phone size={15} />Emergency Contacts</h3>
                                {[
                                    ...
                                    <div key={role} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f8fafc' }}>
                                        ...
                                        <span style={{ fontSize: 13, color: '#ea580c', fontFamily: 'monospace', fontWeight: 700 }}>{num}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
